...
import "../Libraries/_0xAether.sol";

import "./IAetherHubV2.sol";


contract V2AetherHub is AetherCtx, IAetherHubV2 {

	using _0xAether for bytes;

	mapping( bytes16 => AetherTxn ) internal aether_transactions;
	mapping( uint256 => bytes16 ) internal txn_nonce;
	...
	function aetheringChains() public view returns( uint256[] memory ){

		return _aetheringChains;
	}

	function roll() public view returns( uint256 roll_cid ) {

		return _aetheringChains[_rollIdx()];
	}

	function _rollIdx() internal view returns( uint256 ){

		return ( _nodeRegisteryPtr%( _aetheringChains.length - 1 ) );
	}
	...
	function execBlock( bytes calldata aBlock ) external override returns( bool ) {
		...
		require( _checkSort( _ablockTxnHashes ) );
	...
	function _checkSort( bytes16[] memory txn_hashes ) internal view returns( bool ) {
