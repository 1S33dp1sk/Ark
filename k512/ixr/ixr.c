...
ixr_h *ixr_set(d_into *ist) {
	ixr_h *header=__header__();
	char const *var_name=(char const *)din_arg_n(ist, 1);
	char const *var_val=(char const *)din_arg_n(ist, 2);
	#ifdef PROCESS
		printf("@ATP<set>(%s){%s}\n", var_name, var_val);
	#endif
	return header;
...
ixr_h *ixr_prun(d_into *ist) {
	...
	char const *p_name=(char const *)din_arg_n(ist, 1);
	#ifdef PROCESS
		printf("@-Protocol : run    :: %s\n", p_name);
	#endif
	return header;
};
...
ixr_h *ixr_psave(d_into *ist) {
	ixr_h *header=__header__();
	char  const *d_name=(char const *)din_arg_n(ist, 1);
	#ifdef PROCESS
		printf("@-Protocol : save   :: %s\n", d_name);
	#endif
	return header;
};
...
ixr_h *ixr_pcollect(d_into *ist) {
	ixr_h *header=__header__();
	char  const *f_att=(char const *)din_arg_n(ist, 1);
	#ifdef PROCESS
		printf("@-Protocol : collect :: %s\n", f_att);
	#endif
	return header;
};
...
ixr_h *args3head(d_into *in_args) {
	switch(din_switch(in_args)) {
		case 2:
			return ixr_get(in_args);
		case 3:
			return ixr_set(in_args);
		case 4:
			return ixr_prun(in_args);
		case 5:
			return ixr_pcollect(in_args);
		case 6:
			return ixr_psave(in_args);
		default:
			return __header__();
	};
};
