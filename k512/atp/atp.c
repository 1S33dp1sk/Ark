...
@-protocol
	...
	char const *decode_pointer(d_into *into) {
		...
		char const *arg=din_argument(into);
	...
	char *decode_point(d_into *into) {
		...
		#endif
		ulong pnt_offset=sep_offset(din_argument(into), d_charms);
		char *point=str_a4offset(din_argument(into), pnt_offset);
		return strdup(point);
	};
...
void *__arc__(aip_arc *st) {
	...
	aip_sock __arcsocket() {
		aip_sock __sok; memset(&__sok,0,sizeof(aip_sock));
...
void *__aip_sock__(aip_sock *sok) {
	...
	int les(void *args) {
		...
		for(temp=__servinfo;temp!=NULL;temp=temp->ai_next){
			...
			if(setsockopt(sockfd,SOL_SOCKET,SO_REUSEADDR,&__yes,2)==-1){
				#ifdef DEBUG
					perror("setsockopt\n");
				#endif
				exit(1);
