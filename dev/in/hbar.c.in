...
unsigned hash4(unsigned level,void const *tohash,size_t thsize) {
    uint8_t const *__ = hashof_raw(level,tohash,thsize);
    unsigned res=0;
    for(int i=0;i<4;i++){
        res|=(unsigned)__[i]<<(i*8);
    }
    return res;
}
/**
 ...
 * 
**/
unsigned long hash8(unsigned level,void const *tohash,size_t thsize) {
    uint8_t const *__ = hashof_raw(level,tohash,thsize);
    ulong res=0;
    for(int i=0;i<8;i++){
        res|=(ulong)__[i]<<(i*8);
 ...
 *
**/
uint8_t const *fhash_raw(unsigned level,char const*filepath) {
    struct stat __st;
    int __fd = open( filepath , O_RDONLY );
    char *fl_hash;
    ...
    return hashof_raw( level , fl_hash , b_read );
}   
/**
file-hash4
 * 
**/
unsigned fhash4(unsigned level,void const *tohash,size_t thsize) {
    uint8_t const *__ = hashof_raw(level,tohash,thsize);
    unsigned res=0;
    for(int i=0;i<4;i++){
        res|=(unsigned)__[i]<<(i*8);
 ...
 * 
**/
unsigned long fhash8(unsigned level,char const*filepath) {
    uint8_t const *__ = fhash_raw(level,filepath);
    ulong res=0;
    for(int i=0;i<8;i++){
        res|=(ulong)__[i]<<(i*8);
    }
