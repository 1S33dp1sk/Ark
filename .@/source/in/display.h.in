...
/** supposedly the window handler. **/
    ...
    #define __L_ARROW_D "»" // S__EXEC
    #define __WIDE_SEP "—" // S__EP
    #define __SPOT_L1 "·"
    #define __SPOT_L10 "•"
    #define __CHECK "[\u2713]"
    #define __NOCHECK "[\u2716]"

    #define AT_DIR_FOUND "@source directory found.\n"
    #define AT_DIR_NOT_FOUND "No @source directory was found.\n"
    ...
    size_t fd_log(unsigned fd,uchar *buf,size_t size){
        size_t b_written = write(fd,buf,size);
        close(fd);
        return b_written;
    }

    size_t f_log(char *l_path,uchar *log_buf,size_t l){
        int __fd = open(l_path,O_WRONLY);
        if(__fd<=0||l<=1){return 0;}
        return fd_log(__fd,log_buf,l);
    } 
