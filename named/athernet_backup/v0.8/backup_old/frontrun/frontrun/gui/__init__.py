...
class UIUX:
	...
	def _buildFInfo(self):
		...
		Label(self.info_frame,text=self.general_info[2]+' USD',fg=self.text_color,bg=self.bg_text,font=self.font_style, padx=50).place(x=15,y=300)
		...
		Label(self.info_frame,text=self.general_info[5]+' trade',fg=self.text_color,bg=self.bg_text,font=self.font_style, padx=50).place(x=700,y=300)
		...
		Label(self.info_frame,text=self.general_info[8]+' token',fg=self.text_color,bg=self.bg_text,font=self.font_style, padx=50).place(x=350,y=450)
	...
	def _buildFTransactions(self):
		...
		transactions = ttk.Treeview(transactions_list_frame,yscrollcommand = transactions_scroll.set)
		...
		transactions.column('ID',anchor=CENTER, width=30)
		transactions.column('token_name',anchor=W, width=90)
		transactions.column('txn_in',anchor=W, width=190)
		transactions.column('txn_in_price',anchor=CENTER, width=60)
		transactions.column('txn_out',anchor=W, width=190)
		transactions.column('txn_out_price',anchor=CENTER, width=60)
		transactions.column('qty_native',anchor=CENTER, width=75)
		transactions.column('qty_bnb',anchor=CENTER, width=75)
		transactions.column('profit',anchor=CENTER, width=90)
		transactions.column('status',anchor=CENTER, width=60)
		...
		transactions.heading("txn_in_price",text="Buy ($)",anchor=CENTER)
		transactions.heading("txn_out",text="Sell Hash",anchor=W)
		transactions.heading("txn_out_price",text="Sell ($)",anchor=CENTER)
		transactions.heading("qty_native",text="Qty Native",anchor=CENTER)
		transactions.heading("qty_bnb",text="Qty BNB",anchor=CENTER)
		transactions.heading("profit",text="Profit/Token",anchor=CENTER)
	...
	def _buildFParams(self):
		...
		self.address_entry =  Entry(self.params_frame, width=60, background=self.bg_color, fg=self.text_color, font=self.font_style)
	...
	def buildBar(self):
		self.progress_bar = ttk.Progressbar(self.base, orient=HORIZONTAL, length=400, mode='determinate', style = 'primary.Striped.Horizontal.TProgressbar',maximum=100)
		self.progress_bar.place(x=300,y=300)

		self.bar_val = Label(self.base,text="", font=self.font_style,bg=self.bg_color, fg=self.text_color)
		self.bar_val.place(x=720, y=300)

		self.bar_st = Label(self.base, text="",font=self.font_style, bg=self.bg_color, fg=self.text_color)
		self.bar_st.place(x=300, y=350)
		self.bar_val.config(text=str(self.progress_bar['value'])+'%')

	def updateBar(self):
		self.progress_bar['value'] += 1
		if self.progress_bar['value'] < 10 and self.progress_bar['value'] > 0:
			self.bar_st.config(text='Checking Internet Connection...')
		elif self.progress_bar['value'] < 20 and self.progress_bar['value'] > 10:
			self.bar_st.config(text='Checking Mozilla Firefox...')
		elif self.progress_bar['value'] < 30 and self.progress_bar['value'] > 20:
			self.bar_st.config(text='Checking Plugins...')
		elif self.progress_bar['value'] < 40 and self.progress_bar['value'] > 30:
			self.bar_st.config(text='Checking MetaMask...')
		elif self.progress_bar['value'] < 50 and self.progress_bar['value'] > 65:
			self.bar_st.config(text='Checking Database...')
		elif self.progress_bar['value'] < 65 and self.progress_bar['value'] > 80:
			self.bar_st.config(text='Auditing Contract Addresses(Tokens)...')
		elif self.progress_bar['value'] < 80 and self.progress_bar['value'] > 100:
			self.bar_st.config(text='Setting Up FrontRun and Finalizing...')

	def closeBar(self):
		self.progress_bar.destroy()
		self.bar_val.destroy()
		self.bar_st.destroy()
...
if __name__ == '__main__':
	u = UIUX('/home/kj/BOT_front-run/frontrun/',
		(1, 'Front Run', '10', '7', '0x1a0119e9268915Fb085387A4d0bf9C57E696D282', '0', '0', '0', '2'), # general
		[(1, '0x81823', '2.1', '294', '1', 'ref_1', 'MBOX', '0x81824', '2.6', '0.5', 'False')], # txns
		[(1, 'WBNB', 'True', '404.23', '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c'), (3, 'MBOX', 'True', '4.9', '0xasd4f021fd')]) # tokens
