...
int init_record(uns lvl,char const*kei,char const*ftypes,...) {
...
int lbb_get(char const *args[]){
	printf("in get");
}
int lbb_set(char const *args[]){
	printf("in set");
}
...
/**
 * The book 
 * lbb is a shorthand for the little black book
 * 
**/ 
...
int main(int argc,char const*argv[]){
	...
	uns __switch=hash4(0,argv[1],strlen(argv[1]));

	return __switch!=LBB_GET?__switch!=LBB_SET?usage():lbb_set(argv):lbb_get(argv);
